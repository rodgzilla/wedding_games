...
- `index.html` — page structure and all CSS (inline `<style>` block), no separate stylesheet. Loads `game.js` as an ES module. Sizing is viewport-relative so phones never overflow: tiles come from `--tile-size` on `#grid`, derived from `--cols` (the word length, set by `renderGrid()`) and capped at 56px, and keys from `--key-w` on `#keyboard`, sized so the 10-key top row always fits `100vw` and capped at 40px. Never re-introduce fixed pixel widths for tiles or keys. `#intro` is the tutorial, a `position: fixed` full-screen overlay above everything so no part of the board can show through behind it; its `.intro-inner` column is centred with `margin: auto` rather than `justify-content: center`, which centres identically but keeps the overflow scrollable instead of clipping it off the top on a short screen. `#demo-grid` reuses the `#grid` tile rules and only overrides `--tile-size`. A `max-height: 580px` media query tightens the whole screen so the "C'est parti !" button stays above the fold on a phone held sideways.
...
- `game.js` — ES module that imports from `logic.js`, `animation.js` and `intro.js`, and owns all DOM manipulation, fetch calls, and game orchestration (grid rendering, keyboard input, round/sublist progression, recap screen).
- `intro.js` — the landing screen's looping demo game. Replays three scripted guesses (`TABLE`, `PLEUR`, `FLEUR` against the hidden word `FLEUR`) on the real `.tile` markup, typing letter by letter and then flipping with the same stagger `game.js` uses, so a guest who has never played sees the rules happen. The demo word is deliberately *not* one of the words in `words.txt` — picking one from there would spoil a round. Cancellation is a `generation` counter rather than a boolean: a loop suspended mid-`await` when the guest taps "C'est parti !" sees a stale generation and returns instead of painting over the board the real game has since rendered. Under `prefers-reduced-motion: reduce` it still plays every step — the row-by-row progression *is* what the tutorial teaches, so stopping it leaves a guest looking at a static grid and concluding the page is broken. The calm variant drops only the movement: no flip animation, no letter-by-letter typing (the whole word lands at once), and longer pauses between steps.
...
0. **Intro screen.** The page loads with the `#intro` overlay covering the page and the board, keyboard, legend and message hidden (inline `display: none` in the markup). `startIntroDemo()` runs at once while `init()` fetches in the background; the "C'est parti !" handler awaits that same promise before calling `startRound()`, so a slow connection cannot start a round with no target word. `startRound()` sets `started = true`, which is what `handleKey()` checks — without it a key press on the intro would reach `submitGuess()` and shake a row that does not exist yet.
